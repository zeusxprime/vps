...
# bash <(curl -sL https://raw.githubusercontent.com/zeusyx/vps/refs/heads/main/install.sh)
# Não usa cd em /dev/fd ou /proc/... porque esse caminho pode sumir durante a execução.
RAW_BASE_PRIMARY="https://raw.githubusercontent.com/zeusyx/vps/refs/heads/main"
RAW_BASE_FALLBACK="https://raw.githubusercontent.com/zeusyx/vps/main"
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    fail "execute como root: sudo bash <(curl -sL https://raw.githubusercontent.com/zeusyx/vps/refs/heads/main/install.sh)"
  fi
}
